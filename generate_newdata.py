...
def generate_cluster_tsp(dataset_size, tsp_size, n_samples_per_cluster=10, cluster_std=0.03, random_seed=None):
    """
    Generate a random dataset with clusters of points.

    Parameters:
    tsp_size (int): The total number of samples to generate.
    ...
    Returns:
    A numpy array of shape (n_samples, 2) representing the generated dataset.
    """
    ...
    n_clusters = tsp_size // n_samples_per_cluster

    # Generate the cluster centers randomly
    cluster_centers = np.random.uniform(size=(n_clusters, 2))
    
    xy = []
    # Generate the samples by adding noise to each cluster center
    for xx in range(0, dataset_size):
        X = np.zeros((tsp_size, 2))
        for i in range(n_clusters):
            start_idx = i * n_samples_per_cluster
            end_idx = start_idx + n_samples_per_cluster
            X[start_idx:end_idx, :] = np.random.normal(
                loc=cluster_centers[i], scale=cluster_std, size=(n_samples_per_cluster, 2)
            )
...
if __name__ == "__main__":
    # x = generate_cluster_tsp(10, 4, 2)
    # x = generate_tsp_data(10,2)
    # print(np.shape(x))
    
    parser = argparse.ArgumentParser()
    parser.add_argument("--filename", help="Filename of the dataset to create (ignores datadir)")
    parser.add_argument("--data_dir", default='newdata', help="Create datasets in data_dir/problem (default 'data')")
        ...
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                if problem == 'tsp':
                    ...
                    dataset = generate_cluster_tsp(opts.dataset_size, graph_size)
                elif problem == 'vrp':
